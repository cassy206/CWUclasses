# -*- coding: utf-8 -*-
"""
Created on Fri Feb  4 22:35:52 2022
...
#Recursive function compute the gcd
def gcdReccursive(a,b): 
...
def getGcdReccursiveList(listA, listB):
    """Assumes listA and listB  are equivalently sized python lists of ints
    Calls the reccursive function to determine gcd of each pair 
    Returns the following: 
    execution time required 
    list of gcd values
    ...
    #recurisively goes through list and determinse gcd of each pair
    for i in range(len(listA)): 
        curGcd = gcdReccursive(listA[i], listB[i])
        #append this gcd to gcdList 
        recGcdList.append(curGcd)
        #append this numIterations to the numIterations list
        numIterations.append(numRecCalls)
        numRecCalls = 0
       
    recendTime = time.time()
    recExeTime = recendTime - recstartTime
    return recExeTime, recGcdList, numRecCalls
...
recExeTime, recGcdList, numRecCalls = getGcdReccursiveList(listA, listB)
...
plt.figure()
bins = 10
plt.hist(numRecCalls, bins, facecolor = 'blue' , label = 'recursive')
plt.hist(itNumIt, bins, facecolor = 'green' , label = 'iterative')
plt.legend(loc = 'best')
plt.title("Iterative vs Recursive gcd for 500,000 ints")
plt.xlabel('Number of iterations/functions calls required')
plt.ylabel('Num occurences of each iteration/ function call value')
